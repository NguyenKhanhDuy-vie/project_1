...
void SSD1306_DrawBitmap(int16_t x, int16_t y, const unsigned char* bitmap, int16_t w, int16_t h, uint16_t color)
{
    ...
    for(int16_t j=0; j<h; j++, y++)
    {
        for(int16_t i=0; i<w; i++)
        {
            ...
            if(byte & 0x80) SSD1306_DrawPixel(x+i, y, SSD1306_COLOR_BLACK);
        }
    }
}
...
void SSD1306_ClearArea(uint8_t x, uint8_t y, uint8_t width, uint8_t height) {
    ...
    uint8_t bit_height = height;

    for (uint8_t page = page_start; page <= page_end; page++) {
        for (uint8_t i = 0; i < width; i++) {
            uint16_t index = page * SSD1306_WIDTH + x + i;

            if (bit_offset == 0 && height >= 8) {
                // Xóa nguyên trang
                SSD1306_Buffer[index] = 0x00;
                bit_height -= 8;
            } else {
                // Xóa m?t ph?n byte
                uint8_t mask = 0xFF;
                ...
                if (page == page_end && (y + height) % 8 != 0) {
                    uint8_t bits_to_keep = 8 - ((y + height) % 8);
                    mask &= ~(0xFF << bits_to_keep);
                }
